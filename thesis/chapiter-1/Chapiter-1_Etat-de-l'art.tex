...
\newpage
\rhead{}
\addcontentsline{toc}{section}{Chapiter 3 : Conception de la solution proposée}  
\section*{\Huge{Chapiter 3}}
\subsection*{\huge{Conception de la solution proposée}}
\newpage
\rhead{\textsc{Chapiter 3 : Conception de la solution proposée}}
\section{Introduction}
...
Pour cela on utilise des outils inventé par l'homme pour faciliter la tache de la surveillance de cet enfant.
\section{Objectif}
...
La figure \ref{fig:achdelasolu} présente cette architecture \textit{(version 1)} que l'on va implémenté en un 1\textsuperscript{èr} temps pour nous familiariser avec les concepts.
...
Les sources de bruit numérique sont multiples, certaines sont physiques liées à la qualité de l’éclairage, de la scène, la température du capteur, la stabilité du capteur de l'images durant l'acquisition, d'autres apparaissent durant la numérisation de l'information \cite{48}.\\
...
\begin{enumerate}
	...
	\item Le modèle traité l'image puis détermine la température.
...
\begin{enumerate}
	...
	\item Appliquer l'algorithme d'augmentation sur les image thermique entrées.
...
\newpage
\rhead{}
\addcontentsline{toc}{section}{Chapiter 4 : Réalisation de la solution proposée}  
\section*{\Huge{Chapiter 4}}
\subsection*{\huge{Implémentation des solutions proposées}}
\newpage
\rhead{\textsc{Chapiter 4 : Implémentation des solutions proposées}}
...
Voici une capture présente comment on a éliminer les bruits d'une image.
\begin{figure}[h]
	\centering
...
Le choix de “Google Colab” était une nécessité, d’abord l’accès à une vitesse de traitement rapide sachant que notre matériel (ordinateur) n’est pas équipé d’un processeur GPU donc le choix était vite fait, et que nous allons utiliser avec “Colab” comme service et on plus l’accès aux autres services de Google en matière de stockage, rapidité et la gratuite du service.
...
Dans le cas de notre étude on utilise le Dataset proposé dans  \cite{47} qui comporte des image infrarouge avec des niveaux de gris. La taille des image et comme suit: $320 \times 220 \times 1$, mais pour des besoin programmatique et manque de mémoire on a réduit la taille de ces images a $64 \times 64 \times 3$. 

Le problème avec cette Dataset c’est qu’elle n’est pas annoter, c-à-d celle-ci n’est pas conçu ou préparée pour la détermination de la température corporelle .
...
Commençant par importer notre ensemble de données et faire quelque pré-traitement qui consiste à redimensionner les images à une largeur et une hauteur de $64 \times 64$ pixels par image.
...
Et puis on affiche l’évaluation de notre modèle, le calcule ce fait manuellement avec la bibliothèque “Keras”, on affiche la précision de notre modèle pour l’entraînement et pour les tests, comme le montre la figure \ref{fig:prediction}.
